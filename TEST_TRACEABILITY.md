...
| Ready then Start / solo launch repair | per-action lobby rate limiting and launch diagnostics in `lobby/CooperLobby.server.luau`; `verify_lobby_launch_repair.luau` | PARTIAL | Local source regression exists; published solo and invited-party reserved-server launch must pass |
| Server-owned party manifest, host, readiness, tickets | lobby server/config and `verify_milestone1_foundation.luau` | PARTIAL | Real MemoryStore/TeleportService replay, outsider, expiration, and partial-failure tests |
| Shared task scheduler and host-world authority | `CooperFamilyTaskGame.server.luau`, config, foundation verifier | PARTIAL | Immutable eligibility and schema-12 transaction-journal source are present; still need two-client same-frame completion, four-client spam/soak, and fault-injected exactly-once persistence |
| Host-only purchases | family server validation and terminal session checks | PARTIAL | Guest UI/server rejection scenarios, concurrent host/guest request, interrupted purchase transaction |
...
| Sprint authority and mobile input | `CooperMovement.server.luau`, `CooperSprint.client.luau`, verifier | PARTIAL | Scenario/security suite, controller/input switching, physical phone resolutions |
